...
class myCanvas {
    ...
    set isBordered(width) {
        this._isBordered = !this._isBordered
    ...
    clear = (tempAlpha) => {
        ...
        if(tempAlpha) { this._color[3] = tempAlpha } // set color rgb(a) value to temp alpha if one is given
        this.ctx.fillStyle = colorString(this._color)
        this.ctx.fillRect(0, 0, this.width, this.height)
        this._color[3] = alpha // reset color rgba
    }
}
